...
class OHLCV:
    ...
    def add_candle(self, pair, tf, k):
        ...
        if tf not in self.store[pair]:
            self.store[pair][tf] = pd.DataFrame(
        ...
        df = pd.concat(
            [self.store[pair][tf], pd.DataFrame([row])]
...
def high_impact_events():
    ...
    if EVENT_CACHE["last"] and (now - EVENT_CACHE["last"]).total_seconds() < 3600:
...
def check_news(symbol):
    try:
        ...
        symbol = symbol.upper()
        return any(
            symbol in (n.get("headline", "") + n.get("summary", "")).upper()
            for n in news
        )
    except Exception:
        return False
...
def in_session():
    ...
    return any(
        datetime.strptime(f"{s:02d}{m:02d}", "%H%M").time()
        <= now
        <= datetime.strptime(f"{e:02d}{n:02d}", "%H%M").time()
...
async def process_signal(pair, tf, df):
    ...
    if sweep["low"].iloc[idx] and mom.iloc[idx]:
        ...
        stop, tp1, tp2, tp3 = tp_sl_signal(
            close, "long", df["low"].iloc[idx], atr_val, df
        )
        size = int(
            np.floor((ACCOUNT_EQUITY * RISK_PER_TRADE) / abs(close - stop))
        )
        log_signal(
            [
...
# ---------- BOOT ----------
def main():
    logging.info("SMC Bot – Render-ready starting…")
